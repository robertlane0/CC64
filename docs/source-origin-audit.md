...
3. Confirm the adjacent target checkout revision and run its volume-cleanliness
   checker before and after target images.
4. Inspect the tracked-file list for external source archives, generated
   objects, executable images, package caches, and vendored compiler trees.
5. Search tracked source and tests for compiler-project names, copied notice
   blocks, machine-code byte blobs, and external repository URLs.
6. Map every non-obvious implementation choice to a specification or a row in
   `docs/provenance-ledger.md`.
7. Review new tests to ensure expected behavior follows C17 or a pinned
   project contract rather than a host compiler's undocumented output.
8. Record the audit date, commit, bootstrap tool version, commands, and result
   in `docs/status.md`.
9. Reject the release if any implementation dependency has unknown origin or if
